...
class Campaign(models.Model):
    ...
    # Auto-tracked counters (updated by signals)
    ...
    def roi_percentage(self):
        if self.budget and float(self.budget) > 0:
            return round(((float(self.actual_revenue) - float(self.budget)) / float(self.budget)) * 100, 1)
        return 0.0

    @property
    def cost_per_lead(self):
        if self.leads_generated and self.leads_generated > 0:
            return round(float(self.budget) / self.leads_generated, 2)
        return 0.0

    @property
    def conversion_rate(self):
        if self.leads_generated and self.leads_generated > 0:
            return round((self.converted_deals / self.leads_generated) * 100, 1)
        return 0.0
